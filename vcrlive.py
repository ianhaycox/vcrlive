...
def loop():
    ...
    car_setup = ir['SessionInfo']
    if car_setup:
        car_setup_tick = ir.get_session_info_update_by_key('SessionInfo')
        if car_setup_tick != state.last_car_setup_tick:
            state.last_car_setup_tick = car_setup_tick
            print('car setup update count:', car_setup['CurrentSessionNum'])

            driver_info = ir['DriverInfo']
            if driver_info:
                print("driver info", driver_info['Drivers'])

            for i in car_setup['Sessions']:
                if i['SessionNum'] == car_setup['CurrentSessionNum']:
                    print('session',i)
                    break

            
        c = ir['CarIdxClassPosition']
        print(c)
